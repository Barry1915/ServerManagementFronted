<template>
  ...
  <h1 class="server-log-title">133服务器auth日志</h1>  
    </div>  
...
export default {
  ...
  methods:{
    tableRowClassName({row, rowIndex}) {
        if (row.log && row.log.includes('root')) {
          console.log()
          return 'warning-row';
        } else{
          return 'success-row';
    ...
    async fetchLogs(){
      try {
        let url=`http://localhost:5678/authlog/page?page=${this.currentPage}&pageSize=${this.pageSize}`
